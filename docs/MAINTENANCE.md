...
**当前进度（2026-09-04 实测）：已收 37 篇 / arXiv 召回 253 篇，覆盖率约 15%。**
未收候选 217 篇，其中标题即含安全关键词的 95 篇。

各节存量（见 `data/sections.yaml` 的 `stock` 字段）：
...
**总召回 253 篇**（2025-01 至 2026-09，含跨类重叠）。按历史收录率 ~40% 估算，
可收录总量约 100–110 篇。建议按 `2.x 防御层 → 1.1 → 1.3 → 1.5 → 3.x` 顺序补齐
（防御层优先，因为 §2.2/2.4/2.6 几乎空着，而候选里 SeerGuard、OSGuard、WARD、
SnapGuard、BraveGuard、CAPED 等正好对位），每批 15–25 篇，一批一个 PR。

### ⚠️ 补存量前必须确认分页正常

`fetch.py` 会打印「arXiv 报告命中 N 篇」。**若 N 大于实际召回数，说明翻页失效，
结果被截断。** 历史上此处有过严重 bug：硬编码 `start=0` 单次请求，而 arXiv 单次
上限 100 条，导致全区间 253 篇只召回 100 篇、未收候选从 217 篇缩水成 76 篇，
且完全静默。已于 2026-09-04 修复。双周窗口通常 <100 篇不受影响，所以这个问题
只在长区间补存量时才会暴露。
